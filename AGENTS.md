...
- **Bot Telegram thông báo hoàn tất riêng** (22/08): `@Job_tu_dong_email_bao_cao_bot` (id `8946172106`). Mỗi job kết thúc gửi 1 tin "HOÀN TẤT pipeline" kèm mô tả công việc qua secrets `TELEGRAM_DONE_BOT_TOKEN` + `TELEGRAM_DONE_CHAT_ID` (=8891619372) — đã set ở CẢ 2 repo. Script: `notify_done.py` (beer-price-scan, repo root) + `scripts/notify_done.py` (HMIP); nội dung truyền qua env `DONE_TASK_DESC`, step chạy `if: always()` cuối mỗi job. ⚠️ Job app-sync của reconcile.yml trước đây KHÔNG có `actions/checkout` → đã thêm (fix commit f2cec94) vì notify_done.py nằm trong repo. Bot này chạy SONG SONG với notify TG+Discord cũ (bot `@lich_quet_tu_dong_bot`), không thay thế.
