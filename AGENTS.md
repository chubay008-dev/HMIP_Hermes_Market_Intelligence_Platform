...
- **Hai hệ thống notify (tách biệt, cả hai đều có anti-spam):**
  - **Hệ 1 (PI incremental):** `extensions/pi/notifier.py::notify_alert` — cho alert giá thay đổi từ `store_price_point_if_changed`. Anti-spam 3 lớp: (1) min-change (`HMIP_NOTIFY_MIN_PCT`, mặc định 5); (2) **per-sku cooldown** (`HMIP_NOTIFY_SKU_COOLDOWN_MIN`, mặc định 30') — 1 SP đổi giá trên nhiều kênh (Tiki+Shopee+Lazada) cùng lúc → chỉ kênh đầu notify, scope `pi-sku`, key `sku:sku_id|region_id`; (3) cooldown per `(sku,channel,region)` (`HMIP_NOTIFY_COOLDOWN_MIN`, mặc định 360'=6h; `0`=tắt). State BỀN VỮNG (PR #9): ngoài cache in-memory (`_notify_state` + `_sku_notify_state`), mốc lần gửi cuối còn ghi vào bảng `pi_notify_state` (PI DB, scope="pi" / "pi-sku") qua `pi_store.set_notify_state`; khi process restart (cache rỗng) → đọc lại từ DB → cooldown vẫn sống (Render free tier ngủ/restart không gây re-notify toàn bộ alert). Chỉ ghi nhận khi ≥1 kênh gửi thành công (fail → alert sau thử lại).
  - **Hệ 2 (PRC-001 scheduler):** `extensions/notifiers/notify_all` (telegram.py + discord.py) — cho alert từ `scheduler.scan_once` → `run_prc_001`. Anti-spam 3 lớp tại `extensions/notifiers/rate_limit.py::allow` (funnel chung): min-change (`HMIP_SCAN_NOTIFY_MIN_PCT`, mặc định 10 → chỉ ESCALATE đi qua) + cooldown per sản phẩm (`HMIP_SCAN_NOTIFY_COOLDOWN_MIN`, mặc định 360'=6h; `0`=tắt; giá đổi ≥5% thì vẫn báo) + no-repeat (cùng decision+giá → skip). State BỀN VỮNG (PR #9): `mark_sent` ghi vào `pi_notify_state` (scope="scan") + cache in-memory; restart → đọc DB → cooldown sống qua restart. Severity trong collector (hệ 1) tính theo magnitude thực (CRITICAL≥10/HIGH≥5/MEDIUM≥3/LOW), không cứng CRITICAL.
