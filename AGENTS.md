...
- **Fix dead man's switch chết theo GitHub:** `scripts/external_watchdog.py` chạy trên **OpenHands Cloud Automation — preset prompt** (id `604c16db-ca60-4bd2-b7ab-ec32eb41844e`, cron `15 5-9,11,14 * * *` Asia/Ho_Chi_Minh — 7 tick/ngày; mỗi tick = 1 conversation nhỏ clone repo + chạy script). Logic: repo nào thiếu run schedule hôm nay (VN) và chưa có dispatch → dispatch chạy bù; beer-scan trước, HMIP reconcile chỉ sau khi scan HOÀN TẤT (tránh race deploy); chống false-positive free-tier delay 2,5-3h bằng guard: chỉ dispatch khi có incident Actions ≤30h qua HOẶC đã ≥08:00 VN; Actions đang degraded → chờ tick sau. Chỉ cần secret `GITHUB_TOKEN`.
- **⚠️ Quirk OpenHands Automation runtime (đo thực tế 28/08):**
  - **Custom script (tarball) KHÔNG dùng được cho việc cần user secrets:** sandbox script-run không inject custom secrets của user — store `GET /api/settings/secrets` rỗng, env không có GITHUB_TOKEN. Cũng không inject `AGENT_SERVER_URL`/`AUTOMATION_CALLBACK_*` như tài liệu; run status theo EXIT CODE (0=COMPLETED); stdout chỉ đọc được qua `error_detail` của run FAILED; sandbox provision ~10-12'/run.
  - **Preset prompt (conversation) THÌ CÓ secrets:** conversation nhận custom secrets như conversation thường, NHƯNG tên HOA được inject thành env **chữ thường** (`GITHUB_TOKEN` → `github_token`). Script đã đọc cả 2 dạng. Verify end-to-end 28/08 ~10:51 VN: run COMPLETED, script exit 0, quyết định đúng (ok/ok, không dispatch thừa).
  - Đọc kết quả 1 run: `GET /api/automation/v1/{id}/runs` lấy `conversation_id` → `GET /api/v1/conversation/{conv_id}/events/search?sort_order=TIMESTAMP_DESC` → message cuối của agent (trong `thought` của ActionEvent cuối).
